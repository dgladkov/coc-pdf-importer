...
console.log(`Current version: ${current}` + (pkg.version !== current ? ` (package.json says ${pkg.version})` : ''));
...
if (!SEMVER.test(version)) fail(`"${version}" is not a MAJOR.MINOR.PATCH version.`);
...
console.log(`Now push main: the release workflow tags ${version}, builds and publishes it.`);
